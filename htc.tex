...
\begin{frame}{\HTC\ \ Syntax}
  ...
  \item<2-> \structure{Example} \ ``$x - y \leq d$'' with $x, y \in \Variables$ and $d \in \Domain$
    \medskip
...
\begin{frame}{\HTC\ \ Semantics}
  ...
  \item \structure{Valuation} \ $\val: \Variables \rightarrow {\Domain \cup \{\Undefined\}}$
    ...
    \end{itemize}
  \item[] \structure{Set-based representation} \ $\val \subseteq \Variables \times \Domain$
    \begin{itemize}
    \item $(x,c)\in\val$ and $(x,d)\in\val$ implies $c = d$
    \item $(x,d)\notin\val$ if $\val(x)=\Undefined$
    \end{itemize}
  \item[] $\mathcal{V}$ is the set of all valuations over \Variables\ and \Domain\
    \medskip
  \item<2-> \structure{Atom denotation} \ $\den{\cdot}: \Atoms \rightarrow 2^{\mathcal{V}}$
